import React from 'react'

function RosierMatters() {
  return (
    <div className='w-full h-70 md:h-[80vh] lg:h-screen flex flex-col items-center justify-center '>
      <div className='w-full text-center text-4xl font-extrabold lg:text-8xl lg:font-bold uppercase md:text-7xl'>
        <h1>Why the rosier</h1>
        <h1>matters</h1>
      </div>
      <div className='w-full text-center mt-3 lg:mt-5 text-[10px] uppercase font-semibold lg:text-sm'>
        <h1>Rooted in tradition, driven by trust.</h1>
        <h1>We are reviving the traditional ways of old Bharat</h1>
      </div>
    </div>
  );
}

export default RosierMatters
